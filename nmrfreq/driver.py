...
class Driver:
    ...
    def defineUsage(self):
        ...
        g.add_argument('-e', '--energy', type=float, nargs='*',
                       help='desired beam energy [MeV]')
        g.add_argument('-f', '--freq', type=float, nargs=1,
                       help='desired NMR frequency [MHz]')
    ...
    def createIsotope(self):
        if (self.isotope is None and
                (self.energy is not None or
                 self.freq is not None or self.field is not None)):
            self.isotope = ['H1']
    ...
    def performCalculation(self):
        self.calc = NMRcalc(
            self.isotope, self.charge, self.energy, self.freq)
